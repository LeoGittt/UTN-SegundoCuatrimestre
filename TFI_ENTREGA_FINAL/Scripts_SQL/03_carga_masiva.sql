...
-- Acá cargamos 20.000 registros para las pruebas de performance
-- Usamos generate_series() que es súper eficiente en PostgreSQL
...
-- Recrear tablas si no existen (por seguridad)
...
SELECT 
    ...
    CASE (i % 20) -- 20 clientes diferentes para análisis realista
        ...
        ELSE 'Camila Ortega'
    END,
    (random() * 8000 + 50)::decimal(10,2), -- Rango amplio: $50-$8050
    CASE (i % 3)
        WHEN 0 THEN 'NUEVO'
...
-- =========================================================================
-- CARGA DE 20,000 ENVÍOS (RELACIÓN 1:1)
-- =========================================================================
INSERT INTO envios (
...
SELECT 
    ...
    (random() * 500 + 30)::decimal(8,2), -- Costos variados: $30-$530
    CASE (p.id % 3)
        WHEN 0 THEN 'OCA'
    ...
    fc.fecha_despacho_calc + (random() * 7 + 3)::int, -- Fecha estimada posterior
    p.id
FROM pedidos p
...
-- Verificación principal de carga
SELECT 
...
-- Ejemplos aleatorios para validación
SELECT 
    'MUESTRA ALEATORIA DE DATOS' as seccion,
